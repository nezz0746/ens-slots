...
export function useBatch() {
  ...
  async function sendBatch(
    ...
    calls: readonly { address: `0x${string}`; abi: any; functionName: string; args: readonly any[] }[],
  ) {
...
function readReason(e: unknown): string {
   ...
   */
  if (/connector not connected/i.test(first))
    return "Connect a wallet first.";
  if (/chain.*mismatch|does not match the target chain|chain of the connection/i.test(first))
